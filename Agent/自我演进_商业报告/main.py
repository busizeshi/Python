...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="自我演进商业报告生成（Google 搜索 + 千问）"
    )
    ap.add_argument("--prompt", type=str, default="分析苹果公司最新商业战略", help="输入提示词")
    ap.add_argument("--steps", type=int, default=5, help="最大迭代步数")
    ...
    if meta.get("search_summary"):
        ...
        for search in meta["search_summary"]:
            print(
                f"  步骤 {search['step']}: {search['query']} "
                f"(找到 {len(search['results'])} 个结果)"
            )
